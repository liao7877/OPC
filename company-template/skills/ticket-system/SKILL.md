...
>
> **⚠️ 路径基准**：工单命令统一经稳定锚 `../../../companies/C00x/workbench/`（从员工目录起算）；已 `cd` 到公司根则直接用 `workbench/` 前缀。**不要从员工目录裸用 `workbench/`**。
...
- **用户直派自助取号**（先占台账后建单，防并发撞号）：
  ...
  2. 建单：`python ../../../opc_tickets.py --company C00x --new <号> "<标题>" --owner <你的E号>`，task.md 备注 `来源: 用户直派`；
