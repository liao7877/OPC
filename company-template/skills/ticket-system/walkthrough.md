...
1. **建单**（编号找总管确认后执行）：
   ```
   python ../../../opc_tickets.py --company C00x --new TSK00010 搭建日志系统 --owner E0001 --project P0001
   ```
2. **开工**：打开 `opc://company:C00x/workbench/tasks/TSK00010-搭建日志系统/task.md`，`status: in_progress`、`priority: 高`、填 `due`，更新 `updated`。
