...
## 行为规范
- 私有技能（渐进式披露）：技能引用走 `opc://company:<本司ID>/skill/<名称>`（见 opc-namespace-design.md）；本员工骨架自带 `skills/INDEX.md`（**生成物**：`python opc_model.py --sync-index` 产出，勿手改），命中触发词再读对应 SKILL.md 全文，勿整份预加载；手动打开技能文件时把 `opc://company:<本司ID>/...` 当 `companies/<本司ID>` 用（真实目录），或 `python opc_resolver.py --resolve <uri>` 取绝对路径（见 opc-namespace-design.md §3.1）。
...
- **知识库（2026-08-30 决策 #19，依据 PRINCIPLES P33）**：本目录 `knowledge/` = 你的**员工级知识库（可写）**；团队级与公司级对你**只读**。
  - **与 `memory/` 分工**：随手写 → `memory/`（时序流水，细节优先，**不进索引、不上浮**）；提炼后下次还用得上 → `knowledge/`（进索引，可上浮）。
  - **查询（三级加载）**：先读本目录 `INDEX.md` 索引行 → 命中才读条目 `summary` → 确认有用才读全文；自己没有再往上查团队级 → 公司级，**由近及远、命中即停**。
  - **上浮**：条目脱离你仍成立 + ≥2 个同事也用得上 → 向 lead / 总管发起（**不要自己往上级目录写**）；上浮 = 物理搬家 + 原处留真实存根，你仍是 `author`（署名答疑），`maintainer` 归接手管理员。
  - **只读纪律**：上级知识（团队 / 公司）**只读**；要定制就 COW 复制到本目录并标 `derived_from`，**源不变**；制度类不可覆盖，只能申请改源头。
