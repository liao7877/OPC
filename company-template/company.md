...
## 全局规范（文件系统即领域模型）
- **命名规范**：实体目录名 = `{前缀}{序号}-{名称}`，其中员工含类别中缀 `{ID}-AI员工-{岗位}`（如 `E0001-AI员工-<岗位>`；生成器解析显示名时自动剥离中缀）；团队/项目为 `{ID}-{名称}`（如 `T001-AI开发团队`、`P0001-示例项目`）——目录自解释：打开仓库即知谁是谁；序号位数：C/T 三位（C001/T001）、E/P 四位（E0000/P0001）、TSK 五位（TSK00001）；公司目录同名法 `C00X-<名称>`
