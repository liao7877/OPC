...
## 全局规范（文件系统即领域模型）
- **命名规范**：实体目录名 = `{前缀}{序号}`（ID-only，决策 #17——显示名与物理路径解耦：员工显示名唯一真相在 roster「岗位」列，团队/项目在 team.md/project.md「名称」字段，改显示名不动目录）；序号位数：C/T 三位（C001/T001）、E/P 四位（E0000/P0001）、TSK 五位（TSK00001）；公司目录例外，保留 `C00X-<名称>`（公司改名由稳定锚机制消化）
  - 遗留兼容：带 `-名称` 后缀的旧目录名仍被识别，但显示名一律取自登记字段、永不取自目录名
