...
扫描 workbench/tasks/ 下各工单目录，解析 task.md 的 frontmatter + 正文，
内联 messages.md / deliverables/ / logs/，产出 tasks-data.json。

用法：
    python generate_tasks.py            # 单次生成
    python generate_tasks.py --watch    # 监听 tasks/ 变动，自动重跑

设计要点：
- 单一真相：workbench/tasks/ 为唯一数据源。
- 离线友好：详情数据（messages / deliverables / logs）全部内联进 JSON，
  因为 file:// 下浏览器无法运行时 fetch 本地文件。
...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "tasks-data.json")
...
def list_files(directory):
    """列出目录下所有文件（含子目录），返回 {name, rel_path} 列表。"""
    result = []
...
def build_input_item(dirname, idx, path, name):
...
def build_record(task_dir, dirname):
    ...
    # 实际完成时间（用户需求）：status=done 时应填写；未填则告警提示（不跳过）
    ...
    # 前置阻塞（机制扩展 #3，2026-08-28）：blocked_by 单行数组，所列工单全部 done 前不得开工
    ...
    if isinstance(raw_in, list):
        for idx, item in enumerate(raw_in):
            if isinstance(item, dict):
                ...
                inputs.append(build_input_item(dirname, idx, path, name))
            elif isinstance(item, str) and item.strip():
                inputs.append(build_input_item(dirname, idx, item.strip(), ""))
    ...
    return {
        ...
        "deliverables": list_files(os.path.join(task_dir, "deliverables")),
        "logs": list_files(os.path.join(task_dir, "logs")),
        "handoffs": handoffs,
...
def extract_escalations(messages_text):
    """升级信箱识别（机制扩展 #18，2026-08-28）：messages.md 中
...
def generate(tasks_dir=None, output_file=None, js_file=None):
    """扫描 tasks_dir（默认 workbench/tasks/）生成数据文件（默认 tasks-data.json / .js）。
    参数化目录/输出，供 --selftest 用临时目录做集成测试，不污染真实数据。"""
    tasks_dir = tasks_dir or TASKS_DIR
    output_file = output_file or OUTPUT_FILE
    js_file = js_file or os.path.join(SCRIPT_DIR, "tasks-data.js")
    tasks = []
    seen_ids = {}
    if os.path.isdir(tasks_dir):
        ...
        print(f"  [警告] tasks 目录不存在：{TASKS_DIR}")
    ...
    emp_reg, proj_reg = build_registry(tasks)
    ...
    # ---- 关系校验（机制扩展 #3/#5，2026-08-28）：阻塞引用 + 父单闭环，只告警不阻断 ----
    ...
    # 原子写：先写临时文件再 os.replace，避免大文件"写一半被读"
    ...
    _atomic_write(output_file, json.dumps(payload, ensure_ascii=False, indent=2))
    print(f"  [完成] 生成 {len(tasks)} 条工单 -> {output_file}")
    ...
    _atomic_write(js_file, js_text)
    print(f"  [完成] 生成浏览器数据 -> {js_file}")
    return len(tasks)
...
def watch():
    print(f"监听中：{TASKS_DIR} （每 3 秒检测变动，Ctrl+C 退出）")
    last = tasks_mtime()
    safe_generate()
    try:
        while True:
...
def new_task(argv):
    """python generate_tasks.py --new <TSKxxx> <标题> [--owner E0001] [--project P0001]
    生成规范工单模板目录（含 task.md），降低 Agent 建单出错率。"""
    rest = argv[argv.index("--new") + 1:]
    if not rest:
        print("用法：python generate_tasks.py --new <TSKxxx> <标题> [--owner E0001] [--project P0001]")
        return 1
    tid = rest[0]
    ...
    task_dir = os.path.join(TASKS_DIR, f"{tid}-{safe_title}")
    ...
    append_worklog_entry(owner, tid, title, project)
    print("已自动重算看板数据（建单即上板），可打开 kanban.html 查看。")
    safe_generate()
    return 0


def append_worklog_entry(owner, tid, title, project):
    """建单即建账（P0-1，廖哥 2026-08-27 拍板）：指定 owner 建单时，
    自动在其 workspace/worklog.md 追加「计划中」条目（含 ticket）。
    双账从源头一致；员工开工只需推进状态。owner 为空或目录不存在则跳过。"""
    if not owner:
        return
    parent = os.path.dirname(SCRIPT_DIR)
    emp_dirs = [d for d in os.listdir(parent) if d.startswith(owner + "-") and os.path.isdir(os.path.join(parent, d))]
    if not emp_dirs:
        print(f"  [提示] 未找到 {owner} 的员工目录，跳过自动建账（该员工接单时按 worklog-discipline 自行补记）")
        return
    wl = os.path.join(parent, emp_dirs[0], "workspace", "worklog.md")
...
def check_structure():
    ...
    known_extra_file_patterns = [re.compile(p) for p in (
        r"^run_boards\.(bat|sh|command)$", r"^register-task\.ps1$", r"^generate_dashboard\.py$",
    ...
    known_subdirs = {"workbench": {"tasks", "affairs"}}  # workbench 下合法子目录（affairs=常设事务区）
...
def selftest():
    """内置自测：解析 / JSON 数组 / 日期规整 / 状态枚举 / 坏文件容错。全部通过返回 0。"""
    import tempfile
    ok = True
    ...
    check("坏 frontmatter 跳过", build_record(d1, "A-坏文件") is None)
    d2 = os.path.join(tmp, "B-非法状态")
    os.makedirs(d2)
    with open(os.path.join(d2, "task.md"), "w", encoding="utf-8") as fh:
        fh.write("---\nid: TSKB\nstatus: 乱写\n---\n")
    check("非法 status 跳过", build_record(d2, "B-非法状态") is None)
    ...
    # ---- 集成用例：generate() 全链路（临时目录隔离，不碰真实数据）----
    ...
    _iw(os.path.join(itasks, "I4-重复id", "task.md"),
        "---\nid: TSKI1\ntitle: 重复id\nstatus: done\nowner: E0003\ncreated: 2026-08-26\nupdated: 2026-08-26\n---\n")
    ijson, ijs = os.path.join(itmp, "out.json"), os.path.join(itmp, "out.js")
    # 追加：阻塞/父子关系集成用例（R1 上游 done 已解锁未开工、R2 阻塞未清就开工、R3 父单提前 done）
    for sub, md in [
        ("I5-父单需求", "---\nid: TSKI5\ntitle: 父单需求\nstatus: done\ntype: 需求\ncreated: 2026-08-01\nupdated: 2026-08-01\n---\n"),
        ("I6-子单A", "---\nid: TSKI6\ntitle: 子单A\nstatus: backlog\nparent: TSKI5\ncreated: 2026-08-01\nupdated: 2026-08-01\n---\n"),
        ("I7-已解锁", "---\nid: TSKI7\ntitle: 已解锁\nstatus: backlog\nblocked_by: [TSKI1]\ncreated: 2026-08-01\nupdated: 2026-08-01\n---\n"),
    ...
    generate(itasks, ijson, ijs)
    with open(ijson, encoding="utf-8") as fh:
        idata = json.load(fh)
...
if __name__ == "__main__":
    if "--watch" in sys.argv:
        ...
        sys.exit(new_task(sys.argv))
    elif "--selftest" in sys.argv:
        sys.exit(selftest())
    elif "--check-structure" in sys.argv:
