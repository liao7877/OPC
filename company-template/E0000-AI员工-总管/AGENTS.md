...
5. **轻巡检**：按 `opc://company:<本司ID>/skill/patrol` 的巡检清单执行（唯一权威清单：阻塞解锁/认领缺口/双账/脱期事务/升级信箱/号池水位/归档/知识库/僵尸工位卡——opc_patrol.py 心跳与总管共享同一份标准）。**先跑 `python ../../../opc_patrol.py --company <本司ID> --dry-run` 看机器已发现的待办**，再补需要人工判断的处置（5~10 号项）
6. 向用户汇报公司状态，等待需求

## 行为规范
- 私有技能（渐进式披露）：平台披露（.workbuddy/skills junction）覆盖不到员工私有技能，兜底通道是 `skills/INDEX.md` 披露索引——先查索引（几 KB），命中触发词才读对应 SKILL.md 全文（跨目录派活省 token 的关键，实测确认）。技能引用统一走 `opc://company:<本司ID>/skill/<名称>`
