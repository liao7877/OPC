...
## 组织架构
- 团队：T001
- 员工：E0000（总管，调度中枢）、E0001（售前工程师，需求分析 × 售前解决方案）、E0002（项目经理，排期 × 交付跟踪）
- 项目：P0004（公司看板与工作记录系统）
- **新建员工**：总管按 `templates/employee-template/` 模板 + `dispatch-sop` 的「新建员工 SOP」执行（AGENTS.md / CLAUDE.md / skills INDEX / junction 四件套，禁止退回老结构）

## 全局规范（文件系统即领域模型）
- **命名规范**：实体目录名 = `{前缀}{序号}-{名称}`，其中员工含类别中缀 `{ID}-AI员工-{岗位}`（如 `E0001-AI员工-售前工程师`；生成器解析显示名时自动剥离中缀）；团队/项目为 `{ID}-{名称}`（如 `T001-AI开发团队`、`P0004-公司看板与工作记录系统`）——目录自解释：打开仓库即知谁是谁；序号位数：C/T 三位（C001/T001）、E/P 四位（E0000/P0001）、TSK 五位（TSK00001）；公司目录同名法 `C00X-<名称>`
...
../companies/C001/
├── company.md              # 本文件（公司总纲）
├── workbench/tasks/        # 统一任务区：TSK000xx-标题/（任务实体目录）
├── T001-AI开发团队/        # 团队实体（team.md）
├── E0000-AI员工-总管/       # 0 号员工：调度中枢
├── E0001-AI员工-售前工程师/  # 员工实体（售前工程师）
├── E0002-AI员工-项目经理/   # 员工实体（项目经理）
└── P0004-公司看板与工作记录系统/   # 项目实体（project.md + 需求文档）
```

## 任务区约定
- 所有任务统一存放：`workbench/tasks/TSK000xx-标题/`
