...
## 全局规范（文件系统即领域模型）
- **命名规范**：实体目录名 = `{前缀}{序号}-{名称}`，其中员工含类别中缀 `{ID}-AI员工-{岗位}`（如 `E0001-AI员工-售前工程师`；生成器解析显示名时自动剥离中缀）；团队/项目为 `{ID}-{名称}`（如 `T001-AI开发团队`、`P0001-示例项目`）——目录自解释：打开仓库即知谁是谁；序号位数：C/T 三位（C001/T001）、E/P 四位（E0000/P0001）、TSK 五位（TSK00001）；公司目录同名法 `C00X-<名称>`
  - 改名零改动（决策 #17）：目录显示名（后缀）为唯一显示名真相，roster「岗位」列须与之一致（不一致生成器告警）；改说明文字跑 `python opc_resolver.py --rename-entity E0001 新说明` 一条龙（git mv + 链接 + roster + 看板 + 全文旧名改写），手动改名后跑 `--heal-entity-refs` 自愈；引用层（opc:// 符号 / 实体 ID）不受改名影响
- **实体即目录**：公司/团队/员工/项目/任务均为独立目录
- **引用制（图）**：归属关系通过目录内声明表达，支持多对多，不依赖物理嵌套
- **员工位置无关（逻辑归属）**：员工与公司/团队为逻辑引用关系、无嵌套依赖，经 `E0000-AI员工-总管/roster.md` 发现；**物理约束**：参与看板体系的员工目录必须位于公司根目录下（扫描约定，见 P0004 需求文档）
