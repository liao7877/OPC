...
- **待办（E0000 目录改名顺延）**：3 个 ZCode 会话进程 cwd 占用 `E0000-AI员工-总管/`，无法改名。用户关闭这些会话后执行：用户拍板保留自解释目录名（决策 #17 终版），E0000 维持现名即可，无需再改。机制对遗留名完全兼容，不改名也不影响运行。
- 用户验收通过（含目录命名两轮拍板：保留自解释带名目录 + 统一「AI员工-」中缀），工单关闭。
