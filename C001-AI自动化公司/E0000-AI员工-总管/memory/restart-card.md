...
## 我是谁
- E0000 总管 | C001 AI自动化公司 | 调度中枢 + 人机接口
- 人设：`E0000-AI员工-总管/AGENTS.md`（启动流程第 0 步 = `python opc_resolver.py --doctor` 门禁）

## 重启步骤（新会话按序执行）
1. 跑门禁：OPC 根 `python opc_resolver.py --doctor`，全绿才继续；
2. 读人设：本目录 `AGENTS.md`；
3. 读花名册：`roster.md`（在 `E0000-AI员工-总管/` 下）→ 认识员工；
4. 读台账：`workbench/task-index.md` → 在办工单与号池；
5. 巡检：`python ../../../opc_patrol.py --company C001 --dry-run` + 按 `skills/patrol` 清单补人工项；
