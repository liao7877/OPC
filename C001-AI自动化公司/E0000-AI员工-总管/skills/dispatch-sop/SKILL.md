...
- 开工/收尾顺手看各层 `knowledge/` INDEX 跳过清单：无 frontmatter 裸文件 = 待整理资料，按摄入旅程处理（总管是全公司唯一摄入守门人）；沉淀进 knowledge/methods/ 后提示自动消失
- **结构权属默认冻结**：用户搭建的三级知识库目录结构是设计产物——总管/员工登记提炼内容自由，**动结构（增删改名/移动/重分类）必须先 escalate 向用户汇报（动什么/为什么/建议），拍板后执行**；仅用户明示授权自治的部分（各级 KB.md 权属段有登记）由责任人自行管理
- 授权登记：用户说「这个目录归 xx 自治」→ 在该层 KB.md 权属段登记 `<子目录> → 授权 <E000x>（日期）`；无登记 = 冻结
