...
- **company.md 锚点的维护边界（回答「总得有人维护吗」）**：仅公司根目录一份 `company.md`，其「公司 ID」由 `create-company` 技能在**建公司时写一次**；ID 是位置无关的，**目录改名不需要改它**。子实体（员工/团队/项目）靠目录名前缀（`E0000`/`T001`/`P0001`，即 `{前缀}{序号}-{名称}` 的 ID 段）扫描发现——**裸 ID 与带名目录均兼容，物理目录改名对引用层透明（决策 #17：ID 逻辑锚）**，**无 md、零维护**。锚点漂移由 `audit_structure`（被 `--check` / pre-commit 自动跑）兜底报警——你无需记着维护，工具替你盯。
...
### 6.1b 页内跳转链接的锚前缀退役（2026-08-29，服务模式 404 修复）

§6.1 的锚前缀方案在 **file:// 直开**下正确，但 OPC 服务上线（决策 #18）后出现跳转 404：
服务把 URL 前缀 `/C001/` 已经路由到公司根（`http://127.0.0.1:8765/C001/dashboard.html`），
页面里的锚前缀链接再叠 `../../companies/C001/`，浏览器解析成 `/companies/C001/...`，
服务端按默认租户公司根再拼一次 → `companies/C001/companies/C001/...` 不存在 → 404。
典型症状：驾驶舱 → 团队 → 团队成员（纯相对路径，正常）→ 团队项目（锚前缀深链，没反应）。

**修复**：看板三级页面之间的互跳链接一律改用**纯相对路径**（`../dashboard.html`、`../workbench/kanban.html`），
file:// 与服务两种模式都成立（所有页面同处公司根目录树内，相对关系一致）；
锚前缀仅保留给「跨公司引用」场景，公司内页间导航不再使用（生成器 `sibling_nav_links()`，selftest 有护栏）。
文档层 `opc://` 与跨公司稳定锚引用不受本节影响。
